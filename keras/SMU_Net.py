...
### sele-reversal operation in background-assisted fusion unit
...
def BAF_unit(input_fore, input_back, ker_num):#ker_num=[256,128,64,32]
    #############################################back_path######################################################
    up_back = UpSampling2D(size=(2, 2), interpolation='bilinear')(input_back[1])#1024
    conv_dila_b = conv_unit2(up_back, 1, ker_num, dilation_rate=2)#256
    conv_be = conv_unit2(input_back[0], 1, ker_num // 2)#128
    conv_bd = conv_unit2(input_back[2], 1, ker_num // 2)#128
    conca_b1 = concatenate([conv_be, conv_bd], axis=-1)#256
    add_fuse_b = Add()([conv_dila_b, conca_b1])
    add_fuse_b = Activation(activation='relu')(add_fuse_b)#256

    conv_b1 = conv_unit(add_fuse_b, 1, ker_num)#256
    conv_b2 = conv_unit(conv_b1, 3, ker_num)#256
    conv_b3 = conv_unit(conv_b2, 1, ker_num//8)#128
    conca_b2 = concatenate([add_fuse_b, conv_b3], axis=-1)#256+128
